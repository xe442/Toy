...
![image](https://github.com/xe442/Toy/blob/master/总复习图片/类图.png)
...
顺序图的定义：
顺序图是一种交互图，由一组对象及这些对象之间的通信组成。注重于描述消息的次序，可以表示一组对象之间的交互情况。
...
![image](https://github.com/xe442/Toy/blob/master/总复习图片/顺序图.png)

有四种结构化控制操作符：
1. 选择执行opt [condition]，只有在condition为真时才执行
2. 条件执行alt [condition] ... [else] ...相当于if...else...的执行流程
3. 并发执行par，其中会有虚线将这一块分成多个部分，部分与部分之间并行执行。
4. 迭代执行loop(m, n)，做n - m次操作。
